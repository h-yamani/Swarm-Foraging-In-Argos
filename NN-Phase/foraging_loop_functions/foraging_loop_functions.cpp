...
void CForagingLoopFunctions::Init(TConfigurationNode& t_node) {
   try {
      ...
      num_forager=7;
      foragers=0;
      //UInt32 unFoodItems1=20,unFoodItems2=9,unFoodItems3=9;
      unFoodItems4=10;
      ...
      m_cForagingArenaSideX.SetMin(-3.5f);
      m_cForagingArenaSideX.SetMax(3.5f);
      m_cForagingArenaSideY.SetMax(3.5f);
      m_cForagingArenaSideY.SetMin(-3.5f);
      Real x,y,v,vcount=0;//,newx,newy,sign=1,j=0;
      //CRadians rx,ry;
      //for(UInt32 j = 0; j < 3; ++j) {
       for(UInt32 i = 0; i < unFoodItems4;i++) {
        x=m_pcRNG->Uniform(m_cForagingArenaSideX);
        y=m_pcRNG->Uniform(m_cForagingArenaSideY);
        if((x<=2.0f &&x>=-2.0f) &&( y<=2.0f &&y>=-2.0f)){
         i--;}
          ...
          //newx = x;
          //newy = y;
          std::cout<<x<<","<<y<<"v"<<v<<std::endl;
          m_cFoodPos.push_back(CVector2(x,y));
          unFoodItems.push_back(CVector2(v,0));
          vcount +=v;
         /* for(UInt32 k = 0; k < v; ++k) {
             m_cFoodPos.push_back(CVector2(newx,newy));
             sign *=-1;
             j++;
             newx = x; //+ 0.02f*j*sign;
             newy = y; //+ 0.02f*j;
     ...
     std::cout<<vcount<<std::endl;   
      ...
      m_cRandomClock.SetMin(3000);
      m_cRandomClock.SetMax(5000);
...
void CForagingLoopFunctions::PrePhysicsEngineStep() {
   ...
   for(CSpace::TAnyEntityMap::iterator it = m_cFootbots.begin();
      ...
      if((m_cSpace.GetSimulationClock()%500)==0 && m_cSpace.GetSimulationClock()!=0 ){
              cController.SetHiveTime();
      }
